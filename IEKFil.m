function [squared_gospa_t_tot,squared_gospa_loc_t_tot,squared_gospa_mis_t_tot,squared_gospa_false_t_tot] = IEKFil(X_truth,z,m_b,t_birth,t_death,P_ini,p_s_constant,it,KLD,kld,Cells,cell_width,n,F,Q,R,Nsteps,sensor_pos,sensor_index,sensor_num)
...
for k=1 : Nsteps
    ...
    for i=1:L % Iterate over each potential target
        ...
        [mean_y(:,i),H]=sensorMeasurement(mean_pred(:,i),sensor_num,sensor_pos,sensor_index,Cells);
        mean_yy(:,:,i)=mean_y(:,i)*mean_y(:,i)';
   ...
   W=P_pred;
    ...
    for i=1 : L % Iterate over each potential target
        for j=1:it
           ...
           [mean_y(:,i),H]=sensorMeasurement(u_p(:,i),sensor_num,sensor_pos,sensor_index,Cells);
          
            %Find A, b, omega
            A= (W(:,:,i)*H')'/W(:,:,i);
            b= mean_y(:,i)-A*u_p(:,i);
            omega=zeros(sensor_num,sensor_num);
